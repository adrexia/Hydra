...
class Database{
   ...
   ************************************************************/
	public function __construct(){   
		try{
            ...
            throw new Exception ("Sorry, there's been an error connecting to the database");
			}    
		}catch(Exception $e) {
			die($e->getMessage());
		}	
   ...
   *************************************************************************************/
   public function getAllUserNames(){
      $qry = "SELECT userID, userName, userFullName FROM hy_user ORDER BY userFullName";
   ...
   ******************************************************************************************/
   public function getUserInfo($userID){
	 ...
	 /********************************************************************************************
    * Method grabs all details supplied in all user's registration
   ******************************************************************************************/
   public function getRegistrations(){
      $qry = "SELECT hy_info.userID, userName, userFullName, userEmail, infoMemberShip, infoAttend, infoAccom, infoPlayWith, infoNotPlayWith, infoTransport, infoFood, infoComments FROM hy_info LEFT JOIN hy_user ON hy_info.userID=hy_user.userID ORDER BY infoID";
      $results=$this->get($qry);          
      return $results;  
   }//end getUserInfo
   ...
   ******************************************************************************************/
   public function getUserRegistration($userID){
      $qry = "SELECT hy_info.userID, userName, userFullName, userType, infoMemberShip, infoAttend, infoAccom, infoPlayWith, infoNotPlayWith, infoTransport, infoFood, infoComments FROM hy_info LEFT JOIN hy_user ON hy_info.userID=hy_user.userID WHERE hy_user.userID='$userID'";
      $results=$this->get($qry, 1);          
      return $results;  
   }//end getUserInfo
   ...
   ***********************************************************************************************/	
	...
	********************************************************************/
	public function getPlayersForGame($gameID, $confirmed=""){
		if($confirmed==""){
			...
			$qry = "SELECT hy_user.userID, userName, userFullName FROM hy_usergames LEFT JOIN hy_user ON hy_usergames.userID=hy_user.userID WHERE gameID='$gameID'";
		}	
		$results=$this->get($qry);    
		return $results; 
	...
	********************************************************************/ 
	public function getGames(){
		$qry = "SELECT gameID, userID, gameName, gameDescription, gameCostume, gameMaxPlayers, gameRestriction, gameGenre, gameExtraInfo, gameStatus, gameDateUpdated, gameSlot, gameVenue, gameNumCast, gameAuthor FROM hy_games ORDER BY -gameDateUpdated";
      $results=$this->get($qry);    
      return $results;
	}//end getGames
	...
	********************************************************************/
	...
	public function getGameStatus($gameID){
		$gameStatus=$this->getGameFromID($gameID);
 ...
 /****************************************************************************************************
   ...
   *********************************************************************************************************/
	...
	/********************************************************************************************************
    *Method writes users game choices to the usergames table
   *********************************************************************************************************/
	public function putUserGames($userID, $gameID, $userGamesPref, $userGamesCharPref, $userGameStatus="pending"){
		if(!get_magic_quotes_gpc()){
   ...
   *********************************************************************************************************/
	...
	******************************************************************************/
	...
	public function updateUserType($userID, $userType){
   		$qry = "UPDATE hy_user SET userType = '$userType' WHERE userID='$userID'";
      	$result=$this->put($qry);
      	return $result;		
   }
